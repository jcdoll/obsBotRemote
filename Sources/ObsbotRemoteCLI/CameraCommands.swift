...
extension CommandLineTool {
  ...
  func runCameraXUGet(arguments: [String]) throws {
    ...
    if options.length == nil,
      let length = try? controller.readExtensionUnitLength(unitID: unitID, selector: selector)
    {
      print("length=\(length)")
    }
  ...
  func runCameraXUDump(arguments: [String]) throws {
    ...
    for unit in probe.extensionUnits {
      ...
      for selector in unit.advertisedSelectors {
        ...
        let length = try? controller.readExtensionUnitLength(
          unitID: unit.unitID, selector: selector)
