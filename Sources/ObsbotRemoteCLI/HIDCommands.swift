...
extension CommandLineTool {
  ...
  func runControl(arguments: [String]) throws {
    ...
    print(
      "Loaded \(capture.buttons.count) captured button signature(s) from \(defaultRemoteButtonCaptureURL.path)."
    )
    ...
    while true {
      ...
      case .unknown:
        if let heldInput = heldRemoteInput(from: input.terminalBytes) {
          runCameraAction(
            for: heldInput.button, controller: controller, repeatCount: heldInput.repeatCount)
  ...
  func runListen(arguments: [String]) throws {
    ...
    while true {
      ...
      case .ambiguous(let buttons):
        print(
          "ambiguous \(buttons.joined(separator: " / ")) -> \(dryRunActionDescription(for: buttons[0]))"
        )
      case .unknown:
        if let heldInput = heldRemoteInput(from: input.terminalBytes) {
