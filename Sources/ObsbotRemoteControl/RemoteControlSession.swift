...
public enum RemoteControlSessionError: Error, CustomStringConvertible {
  ...
  public var description: String {
    ...
    case .hidOpenFailed(let code, let seize):
...
public final class RemoteControlSession: @unchecked Sendable {
  ...
  public func start() throws {
    ...
    do {
      ...
      let manager = makeHIDManager(
        vendorID: configuration.vendorID, productID: configuration.productID)
      let openOptions =
        configuration.requireSeize
        ? IOOptionBits(kIOHIDOptionsTypeSeizeDevice)
        : IOOptionBits(kIOHIDOptionsTypeNone)

      let result = IOHIDManagerOpen(manager, openOptions)
      guard result == kIOReturnSuccess else {
        throw RemoteControlSessionError.hidOpenFailed(
          code: result, seize: configuration.requireSeize)
      }
      ...
      log(
        "Loaded \(capture.buttons.count) captured button signature(s) from \(configuration.buttonCaptureURL.path)."
      )
  ...
  private func handle(_ input: InputCapture) {
    ...
    case .matched(let button):
      runCameraAction(for: button, controller: controller)
    case .ambiguous(let buttons):
