...
`camera-probe` now prints UVC processing-unit descriptors when the camera advertises them. User-facing image controls present OBSBOT-style `0...100` values with `50` as neutral, then prefer standard UVC processing-unit writes when the camera advertises brightness, contrast, or saturation. The UI maps `50` to the UVC default value, so raw defaults such as `128` or `5912` stay neutral without being exposed as the slider value. It intentionally does not apply UVC current readback on launch because the observed Tiny 3 can report raw/stale maximum values while the live image is still neutral. If standard UVC processing-unit control is unavailable, image adjustments fall back to OBSBOT extension unit 2 selector 2 as 60-byte V3 vendor packets using command ids `0x00A7` for brightness, `0x00B1` for contrast, and `0x00B5` for saturation. White balance uses standard UVC white-balance temperature and auto controls, matching the SDK `cameraSetWhiteBalanceR` UVC branch, which calls `uvcWhiteBalanceTemperatureSet(param, wb_type == DevWhiteBalanceAuto)`. Boolean UVC auto controls are written as raw `0|1` values and are not clamped through `GET_MIN`/`GET_MAX`, because the observed Tiny 3 can report ranges that would clamp `0` back to `1`. An SDK diagnostic against the observed Tiny 3 (`productType=18`, `devMode=uvc`) returned success from `cameraSetWhiteBalanceR(DevWhiteBalanceManual, 5000)` but still reported `DevWhiteBalanceAuto` from `cameraGetWhiteBalanceR`, so CLI white-balance readback is labeled raw rather than semantic. Do not reuse V3 command id `0x0081` for white balance; in the SDK command mapping used here, `0x0081` is the gesture-track command. The current macOS path does not have a confirmed semantic OBSBOT image readback equivalent for the SDK get calls, so `camera-image` reports semantic ranges and only reports current image values when standard UVC processing-unit readback is available.
